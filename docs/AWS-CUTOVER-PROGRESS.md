...
- [ ] **Step 5 — flip DNS** at GoDaddy: change the `portal` CNAME from `gwa-portal.onrender.com` → `d14c1520tin554.cloudfront.net`. Soak 24–48h with Render as instant rollback (flip the CNAME back), then decommission the Render **web** service (keep RDS + S3; keep or migrate the crons).
