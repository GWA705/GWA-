...
- [x] **Real email — SMTP configured (Aug 2026).** Live and sending from
  `hello@ghsbarrie.ca` (confirmed on Admin → System health). All notification
  emails now actually send: office notifications on a customer call, admin alerts
  on a dealer login request, payout receipts to distributors, status-change and
  new-note emails.
