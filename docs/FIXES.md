...
|---|------|-------------|-----|--------|
| 14 | Timestamps | All times (audit log, histories, deal times) showed in UTC, not Eastern — a late-evening login read as next-day 3 a.m. | Server now runs in `America/Toronto` (set in start.sh + render.yaml), so every timestamp renders in local Eastern time (EST/EDT auto). No code sweep, money formatting untouched. | In testing (staging) |
| 13 | One-time payment card | A finance company issues a one-time virtual Visa (starts 4356) the dealer must relay to the reviewer to pay HD. Storing PAN + CVV in the portal is a PCI concern (CVV must never be stored). | **Parked** by decision — not storing card data for now. Handle out-of-band (see note). Options for later: ephemeral encrypted relay (masked, reveal-to-pay, wipe-after-use, 72h auto-purge) or a tokenization vendor (Basis Theory / VGS). | Parked (compliance) |
