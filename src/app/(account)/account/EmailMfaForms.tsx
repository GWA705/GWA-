...
export function StartEmailMfaButton() {
  ...
  return (
    <form action={action} className="space-y-2">
      <Pending label={t('account.useEmailCodes')} busy={t('account.sending')} />
  ...
  return (
    <div className="space-y-3">
      <form action={action} className="space-y-3">
        ...
        <div>
          <label className="label" htmlFor="token">{t('account.enterCodeEmail')}</label>
      ...
      <form action={resendAction}>
        <button type="submit" className="text-xs text-brand-700 hover:underline">{t('account.resendCode')}</button>
        {resend.ok && <span className="ml-2 text-xs text-green-700">{t('account.newCodeSent')}</span>}
      </form>
    </div>
  );
}
