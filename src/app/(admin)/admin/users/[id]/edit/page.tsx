  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="card p-6">
        <EditUserForm
          user={{ id: user.id, name: user.name, email: user.email, role: user.role, dealerId: user.dealerId, isDistributor: user.isDistributor, canUseCalculator: user.canUseCalculator, canViewReports: user.canViewReports, canViewLeadershipReport: user.canViewLeadershipReport, canSearchCustomers: user.canSearchCustomers, canViewDealerSnapshot: user.canViewDealerSnapshot, canViewAllLeads: user.canViewAllLeads, canManageGiftCards: user.canManageGiftCards }}
          dealers={dealers}
        />
      </div>
    </div>
