...
import { buildFinancePenetration } from '@/lib/reporting/financePenetration';
...
export default async function FinanceReportPage({
  ...
  const canAll = await canViewDealerSnapshot(user); // cross-office totals gate
  ...
  return (
    <div className="space-y-5">
      ...
      <SectionHero
        eyebrow={t('financeReport.eyebrow')}
        title={t('financeReport.title')}
        subtitle={t('financeReport.subtitle')}
      />
      ...
      {!reportingJournalEnabled() ? (
        ...
        <FinanceReportView report={await buildFinancePenetration(officeId, year)} />
      )}
    </div>
  );
}
