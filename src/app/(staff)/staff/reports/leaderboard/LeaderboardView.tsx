        ...
        <>
          ...
          <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-[#1a2e44] via-[#22344f] to-[#0e2b5c] shadow-sm">
            <div className="grid grid-cols-1 divide-y divide-white/15 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
              ...
              <div className="px-5 py-4">
                <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/80">{t('leaderboard.fundedDeals')}</div>
                <div className="text-3xl font-extrabold leading-none text-white">{report.totalFunded}</div>
                <div className="mt-1 text-xs text-white/80">{t('leaderboard.ofDeals', { total: report.totalDeals })}</div>
              </div>
              <div className="px-5 py-4">
                <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/80">{t('leaderboard.fundedVolume')}</div>
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 text-[10px] uppercase tracking-wide text-gray-500">
                    ...
                    <th className="px-2 py-2 text-right font-medium">{t('leaderboard.colFunded')}</th>
                    <th className="px-2 py-2 text-right font-medium">{t('leaderboard.colWinRate')}</th>
                ...
                <tbody>
                  {report.rows.map((r, i) => (
                    <tr key={r.name} className="border-t border-gray-100">
                      ...
                      <td className="px-2 py-2 text-right tabular-nums text-gray-800">{r.funded}</td>
                      <td className="px-2 py-2 text-right tabular-nums text-gray-800">{r.winRatePct === null ? '—' : `${r.winRatePct}%`}</td>
