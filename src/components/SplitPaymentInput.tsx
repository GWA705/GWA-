  ...
  return (
    <div className="rounded-lg border border-gray-200 p-3">
      ...
      <label className="flex items-start gap-2 text-sm text-gray-700">
        ...
        <span><b>{t('splitPayment.toggleLabel')}</b> {t('splitPayment.toggleHint')}</span>
      </label>
      ...
      {on && (
        <div className="mt-3 space-y-2 border-t border-gray-100 pt-3">
          ...
          <div className="flex items-center gap-2">
            <div className="input flex-[1.4] cursor-default bg-brand-50 text-sm font-medium text-brand-700">{t('splitPayment.financedAuto')}</div>
            <div className="relative flex-1">
              ...
              <input
                ...
                aria-label={t('splitPayment.financedAmountAria')}
              />
            </div>
          ...
          {others.map((l) => (
            <div key={l.key} className="flex items-center gap-2">
              ...
              >
                <option value="">{t('splitPayment.methodPlaceholder')}</option>
                {NON_FINANCED_SPLIT_METHODS.map((m) => (
                  <option key={m.value} value={m.value}>{paymentMethodLabel(t, m.value)}</option>
                ))}
              </select>
              <div className="relative flex-1">
                <span className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-xs text-gray-400">$</span>
              ...
              <button
                ...
                aria-label={t('splitPayment.removeLine')}
              >
                ✕
              </button>
            </div>
          ...
          <div className={`flex items-center justify-between rounded-md px-3 py-2 text-xs ${over ? 'bg-amber-50 text-amber-800' : 'bg-green-50 text-green-800'}`}>
            <span>{t('splitPayment.dealTotal', { amount: money(total) })}</span>
            <span className="font-semibold">
              {over
                ? t('splitPayment.exceedsTotal', { amount: money(othersSum - total) })
                : t('splitPayment.balanceSummary', { financed: money(financed), other: money(othersSum) })}
            </span>
          </div>
          <p className="text-xs text-gray-400">
            {t('splitPayment.helpText')}
          </p>
        </div>
      )}
    </div>
