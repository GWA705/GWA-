...
export function MonthlyTrend({ months }: { months: { label: string; value: number }[] }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <h3 className="font-bold text-[#0d2a63]">Applications This Month</h3>
      <div className="flex h-[180px] items-end justify-around px-6 pb-4">
        {months.map((m) => (
          <div key={m.label} className="flex h-full flex-col items-center justify-end">
            <div className="mb-2 text-xs font-bold text-slate-700">{m.value}</div>
            <div
            ...
            />
            <div className="mt-2 text-xs text-slate-500">{m.label}</div>
          </div>
        ))}
      </div>
    </div>
